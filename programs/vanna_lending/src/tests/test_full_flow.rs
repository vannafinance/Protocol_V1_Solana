...
/// Happy path: init -> supply -> create margin -> deposit -> borrow -> partial repay -> withdraw -> redeem.
#[test]
fn full_protocol_flow() {
    ...
    let res = send(
        &mut svm,
        &admin,
        &[ix_admin_initialize_reserve(&admin.pubkey(), &admin.pubkey(), &usdc_mint, DEFAULT_RATE_CURVE, 1_000, 0, 0, 0)],
        &[],
    );
    ...
    let res = send(
        &mut svm,
        &admin,
        &[ix_admin_initialize_reserve(&admin.pubkey(), &admin.pubkey(), &wsol_mint, DEFAULT_RATE_CURVE, 1_000, 0, 0, 0)],
        &[],
    );
    ...
    let usdc_reserve_vault = get_associated_token_address(&reserve_usdc, &usdc_mint);
    assert_eq!(token_balance(&svm, &usdc_reserve_vault), 500_000 * 10u64.pow(6));

    // Borrower creates margin and deposits WSOL collateral.
    ...
    let margin_usdc_vault = get_associated_token_address(&margin, &usdc_mint);
    assert_eq!(token_balance(&svm, &margin_usdc_vault), borrow_amount);

    // Partial repay from margin.
    ...
    let borrower_wsol_wallet = get_associated_token_address(&borrower.pubkey(), &wsol_mint);
